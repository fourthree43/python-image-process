'''
Author: 张仕山
Date: 2023-12-04 17:24:37
LastEditors: 张仕山
LastEditTime: 2023-12-05 18:09:51
Description:  
FilePath: \coffcieDifferent.py
'''
...
from lib.GDALWrap import readRaster, array2raster2, readImgDataByDirPath, calculateValueByAxis
...
# 栅格图像组计算变异系数
def newCV(inputPath, outpath):
    bandsData = readImgDataByDirPath(inputPath, ".tif")
    result = calculateValueByAxis(bandsData, coefficient_of_variation, "计算变异系数")
    # 保存栅格图像
    print("结果输出中...")
    fileFullPath = os.path.join(inputPath, os.listdir(inputPath)[0])
    array2raster2(outpath, fileFullPath, result)
    print('栅格图像组变异系数计算完成')

if __name__ == '__main__':
    inputPath = r"F:\project\python\li\data\modis"
    outPath = r"F:\project\python\li\result\modis\VariableCoefficient_ndvi.tif"
    # slope(images, r'E:\斜率2017_2020.tif')
    newCV(inputPath, outPath)
